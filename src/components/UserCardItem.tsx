...
export const UserCardItem : FC<UserCardItemProps> = memo(({ user, handleFollow, handleUnFollow }) => {
  ...
  return (
    ...
    >
      <div 
        className='min-w-0 w-full rounded-2xl bg-white shadow-lg p-4 mb-5 flex flex-col items-start
           transition-all duration-100 hover:-translate-y-0.5 hover:shadow-lg ease-out border border-slate-200 cursor-pointer' 
        onClick={handleClick}
      >
        <p className='truncate font-bold text-2xl mb-2 w-full'>{user.username}</p>
